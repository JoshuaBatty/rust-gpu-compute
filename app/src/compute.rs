...
pub fn start(options: &Options, src_range: Range<u32>) {
    env_logger::init();
    let compiled_shader_modules = maybe_watch(options, None);
    futures::executor::block_on(start_internal(options, src_range, compiled_shader_modules));
}

async fn start_internal(options: &Options, src_range: Range<u32>, compiled_shader_modules: CompiledShaderModules) {
